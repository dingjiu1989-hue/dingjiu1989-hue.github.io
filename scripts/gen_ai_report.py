...
## 8段内容指引
1. **公司概况**：用列表写3-4个业务板块；用编号写2-3个竞争壁垒
2. **财务分析**：用###分营收趋势、盈利能力、资产质量三个子节；每个子节用列表枚举关键**数字**
3. **技术分析**：用列表写价格位置（52周最高/**X**、最低/**Y**、当前/**Z**）；技术指标用列表（RSI、MACD、均线）
4. **市场情绪**：用段落分布研报评级；用列表写资金流向数据
5. **竞品对比**：前2/3用段落分析，后1/3必须加对比表格
6. **估值与健康度**：用列表写PE/PB/PEG估值水平；用段落分析财务健康度；末尾加健康度评分表
