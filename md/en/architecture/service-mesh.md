...
1. **Intercepts all inbound and outbound traffic** using iptables rules.
2. **Applies traffic policies**: routing rules, load balancing, retries, timeouts.
3. **Enforces security**: mutual TLS, authentication policies, authorization policies.
4. **Collects telemetry**: metrics, logs, distributed traces.
5. **Reports health**: connection status, circuit breaker state.
...
| Resource usage | Higher (Envoy is heavier) | Lower (Rust proxy is lightweight) |
| Features | Extensive, many CRDs | Focused, simpler |
| Configuration | Complex, many CRDs | Simple, fewer CRDs |
| mTLS | STRICT, PERMISSIVE, DISABLE | Automatic |
| Traffic splitting | VirtualService + weight | TrafficSplit |
| Community | Large, CNCF | Large, CNCF |
...
1. **Install the control plane** in a namespace separate from applications.
2. **Start with PERMISSIVE mTLS** to avoid breaking existing plaintext connections.
3. **Inject sidecars gradually**: Use namespace-level injection opt-in, not global.
4. **Add observability first**: Use the mesh to gain visibility into traffic patterns.
5. **Enable traffic management**: Start with canary deployments, then add retries and timeouts.
6. **Enable STRICT mTLS**: After verifying all traffic can use mTLS.
7. **Implement authorization**: Add mesh-level authorization policies last.
